...
function emptyInputSignup($articulo, $costo, $tipoArticulo,  $provedor) {
  $result;
  if (empty($articulo) || empty($costo) || empty($tipoArticulo) || empty($provedor) ) {
    $result = true;
  }
  else {
    $result = false;
...
function createUser($conn, $articulo, $costo, $tipoArticulo,  $provedor, $tableInUse, $headerUrlLink) {
  // OLD QUERY
  ...
  $sql = "INSERT INTO ".$tableInUse." (nombre, costo, tipo, idProvedor2) VALUES (?, ?, ?, ?);";

  $stmt = mysqli_stmt_init($conn);

  if (!mysqli_stmt_prepare($stmt, $sql)) {
    // header("location: ../$headerUrlLink?error=stmtfailed");
    echo "Aqui hay un error!! 1112223 -->";
    print_r($sql);
    // exit();
  }
  // $intentos = 0;
  // $hashedPwd = password_hash($pwd, PASSWORD_DEFAULT);
  mysqli_stmt_bind_param($stmt, "ssss", $articulo, $costo, $tipoArticulo, $provedor);
  mysqli_stmt_execute($stmt);
  mysqli_stmt_close($stmt);
